...
from PAdaIN import PermuteAdaptiveInstanceNorm2d
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.padain1 = PermuteAdaptiveInstanceNorm2d(p=0.01)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, 
                                stride=1, padding=1, bias=False)
        self.padain2 = PermuteAdaptiveInstanceNorm2d(p=0.01)
    ...
    def forward(self, x):
        ...
        out = self.bn1(x)
        out = self.conv1(out)  
        out = self.padain1(out)
        out = self.bn2(out)
        out = self.relu(out)
        out = self.conv2(out)
        out = self.padain2(out)
        out = self.bn3(out)
